...
\begin{table}[H]
\centering
\caption{Representative out-of-sample forecasting accuracy on pooled multi-site test splits (corrected PM$_{2.5}$, $\mu$g/m$^3$). Sequence models use TensorFlow-backed Keras training; ``Ridge AR+Fourier'' denotes the restricted autoregressive ridge baseline with seasonal Fourier terms; RF, linear SVR, and XGBoost use tabular lag features. Values match merged pipeline outputs.}
...
24~h & LSTM + multi-head attention & 11.65 & 17.22 & 0.240 \\
24~h & Ridge AR+Fourier & 11.75 & 18.16 & 0.215 \\
24~h & Random Forest (tabular lags) & 14.78 & 19.47 & 0.099 \\
24~h & XGBoost (tabular lags) & 21.37 & 26.32 & $-0.648$ \\
24~h & Linear SVR (tabular lags) & 20.13 & 26.20 & $-0.633$ \\
24~h & Seasonal naïve (weekly anchor) & 14.59 & 21.46 & $-0.094$ \\
\midrule
7~d (168~h) & LSTM & 13.40 & 18.80 & 0.151 \\
7~d (168~h) & LSTM + multi-head attention & 14.82 & 19.96 & 0.042 \\
7~d (168~h) & Random Forest (tabular lags) & 19.68 & 24.76 & $-0.459$ \\
7~d (168~h) & XGBoost (tabular lags) & 41.97 & 49.16 & $-4.749$ \\
7~d (168~h) & Linear SVR (tabular lags) & 38.61 & 42.43 & $-3.283$ \\
...
14~d (336~h) & LSTM + multi-head attention & 15.41 & 21.78 & 0.035 \\
\midrule
14~d (336~h) & Random Forest (tabular lags) & 20.35 & 25.92 & $-0.598$ \\
14~d (336~h) & XGBoost (tabular lags) & 30.56 & 38.14 & $-2.460$ \\
\midrule
28~d (672~h) & LSTM & 15.51 & 21.17 & $-0.194$ \\
28~d (672~h) & LSTM + multi-head attention & 29.77 & 34.80 & $-2.228$ \\
28~d (672~h) & Ridge AR+Fourier & 15.28 & 20.92 & $-0.041$ \\
28~d (672~h) & Random Forest (tabular lags) & 16.47 & 22.33 & $-0.186$ \\
28~d (672~h) & XGBoost (tabular lags) & 19.55 & 25.91 & $-0.597$ \\
28~d (672~h) & Linear SVR (tabular lags) & 25.42 & 30.52 & $-1.215$ \\
\bottomrule
\end{tabular}}
\end{table}
...
24~h & LSTM + multi-head attention & 11.65 & 30.01 \\
24~h & Ridge AR+Fourier & 11.75 & 36.19 \\
24~h & Random Forest (tabular lags) & 14.78 & 26.54 \\
24~h & Linear SVR (tabular lags) & 20.13 & 52.16 \\
24~h & XGBoost (tabular lags) & 21.37 & 23.07 \\
\midrule
7~d (168~h) & LSTM & 13.40 & 31.72 \\
7~d (168~h) & LSTM + multi-head attention & 14.82 & 26.25 \\
7~d (168~h) & Random Forest (tabular lags) & 19.68 & 22.84 \\
7~d (168~h) & Linear SVR (tabular lags) & 38.61 & 71.73 \\
7~d (168~h) & XGBoost (tabular lags) & 41.97 & 28.91 \\
\midrule
14~d (336~h) & LSTM & 14.68 & 31.51 \\
14~d (336~h) & LSTM + multi-head attention & 15.41 & 37.28 \\
14~d (336~h) & Linear SVR (tabular lags) & 13.94 & 27.51 \\
14~d (336~h) & Random Forest (tabular lags) & 20.35 & 24.23 \\
14~d (336~h) & XGBoost (tabular lags) & 30.56 & 21.83 \\
\midrule
28~d (672~h) & LSTM & 15.51 & 32.72 \\
28~d (672~h) & LSTM + multi-head attention & 29.77 & 61.73 \\
28~d (672~h) & Ridge AR+Fourier & 15.28 & 33.49 \\
28~d (672~h) & Random Forest (tabular lags) & 16.47 & 27.00 \\
28~d (672~h) & XGBoost (tabular lags) & 19.55 & 26.84 \\
28~d (672~h) & Linear SVR (tabular lags) & 25.42 & 25.64 \\
\bottomrule
\end{tabular}}
\end{table}
...
At $H=24$~h, vanilla LSTM attains the lowest pooled MAE among sequence models in Table~\ref{tab:forecast-summary} (MAE~=~11.31~$\mu$g/m$^3$; RMSE~=~17.77~$\mu$g/m$^3$; $R^2\approx 0.19$), slightly ahead of LSTM+multi-head attention (MAE~=~11.65~$\mu$g/m$^3$; higher $R^2$) and ridge AR+Fourier (MAE~=~11.75~$\mu$g/m$^3$). Relative to the weekly seasonal naïve benchmark in the same table, MAE for the best sequence model falls by roughly 3~$\mu$g/m$^3$, indicating that learned nonlinear dynamics provide measurable short-horizon value beyond a simple seasonal anchor on this evaluation window. Table~\ref{tab:forecast-upper-decile} shows that MAE$^{90}$ on upper-decile hours is several times larger than pooled MAE for the sequence and ridge rows, while tabular tree models and SVR show the same qualitative pattern. Figure~\ref{fig:scatter_tabular_h24} illustrates for RF, SVR, and XGBoost how favourable pooled accuracy can coexist with visible compression of high-concentration excursions. Harmattan-aware tabular diagnostics on an extended 84-day test window appear in Tables~\ref{tab:forecast-regime}--\ref{tab:tabular-regime-mae}; pooled MAE in Table~\ref{tab:tabular-regime-mae} is not directly comparable to Table~\ref{tab:forecast-summary} because of window length (and stochasticity in tree ensembles).

\subsubsection{Mid-terms (7d and 14d)}\label{sec:res714}
For $H=168$~h, sequence models outperform the weekly seasonal naïve comparator by a clear margin on pooled MAE (Table~\ref{tab:forecast-summary}; Figure~\ref{fig:forecast_mae}). Vanilla LSTM slightly improves upon LSTM+attention on MAE at this horizon (13.40 vs.\ 14.82~$\mu$g/m$^3$), illustrating that attention is not guaranteed to reduce point error at weekly lead times without additional tuning; MAE$^{90}$ remains large for both sequence models (Table~\ref{tab:forecast-upper-decile}), with the gap especially pronounced for linear SVR. Simple naïve benchmarks stratified by Harmattan regime on an 84-day test span are summarised in Table~\ref{tab:forecast-regime}.

At $H=336$~h, rankings diversify: linear SVR on tabular lag features achieves the lowest pooled MAE among the tabulated candidates (13.94~$\mu$g/m$^3$), marginally ahead of LSTM (14.68~$\mu$g/m$^3$). LSTM+attention records higher pooled MAE (15.41~$\mu$g/m$^3$) and only modest pooled $R^2$ (Table~\ref{tab:forecast-summary}). MAE$^{90}$ for LSTM+attention exceeds that for linear SVR in this snapshot (Table~\ref{tab:forecast-upper-decile}), underscoring that medium-horizon rankings depend on whether one emphasises pooled error or stress episodes. Rather than interpreting a single scalar as evidence against sequence modelling broadly, we emphasise that medium-horizon forecasting here approaches the limits of pooled stationarity assumptions under regime mixing; reporting should therefore pair accuracy metrics with regime-aware diagnostics whenever possible. On the 84-day regime-stratified tabular evaluation (Table~\ref{tab:tabular-regime-mae}), ridge AR+Fourier deteriorates sharply in Harmattan relative to pre-Harmattan at this lead, whereas RF and XGBoost remain more stable across regimes (still not comparable numerically to the 28-day pooled table).

\subsubsection{Long-terms (28d)}\label{sec:res672}
At $H=672$~h, pooled $R^2$ is negative for both sequence architectures tabulated (Table~\ref{tab:forecast-summary}), indicating limited incremental variance explained relative to the test-set mean predictor under this split. Pooled MAE is lowest for ridge AR+Fourier (15.28~$\mu$g/m$^3$) and for vanilla LSTM (15.51~$\mu$g/m$^3$), whereas LSTM+attention yields much higher pooled MAE (29.77~$\mu$g/m$^3$) and the largest MAE$^{90}$ among sequence models in Table~\ref{tab:forecast-upper-decile}. Random Forest and XGBoost remain competitive on pooled MAE relative to several baselines. Upper-tail episodes (MAE$^{90}$) remain systematically harder than pooled MAE for all models at this lead, consistent with the concentration extremes in Figure~\ref{fig:pm25_distribution} and with the diagnostic scatter plots at 24~h (Figure~\ref{fig:scatter_tabular_h24}).
