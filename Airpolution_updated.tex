...
Models are evaluated using mean absolute error (MAE), root mean squared error (RMSE), and coefficient of determination (\(R^2\)). To stress-test performance on high-pollution events, we compute upper-decile error (MAE$^{90}$), defined as MAE on test rows where observed PM$_{2.5}$ falls in the top 10\% of the test window. Where the test window spans both regimes, we also report regime-stratified metrics for pre-Harmattan versus Harmattan subsets.
