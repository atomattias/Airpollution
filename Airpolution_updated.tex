...
We benchmark Long Short-Term Memory (LSTM) sequence models against seasonal-na\"{i}ve and Ridge+Fourier autoregressive baselines, and classical machine learning models (linear SVR, Random Forest, XGBoost) using direct forecasting at 24~h, 7~d, 14~d, and 28~d horizons under leakage-safe, label-time blocked splits. We additionally evaluate \emph{joint} multi-horizon training, where one model predicts all horizons simultaneously, to test whether sharing information across lead times improves generalisation. All models export row-level test predictions; uncertainty is quantified with moving-block bootstrap confidence intervals ($B{=}1000$) and pre-specified paired comparisons of $\Delta$MAE on aligned forecast rows. Evaluation reports pooled MAE (with 95\% CIs), upper-decile MAE (MAE$^{90}$), and regime-aware diagnostics where feasible.

Four findings emerge from this limited five-month observational window.

\textbf{First, jointly trained sequence models with multi-head attention achieve the lowest pooled MAE at 7~d, 14~d, and 28~d} (11.32, 9.78, and 13.56~\textmu g/m\textsuperscript{3}; 95\% CIs in Table~\ref{tab:forecast-summary}). At 24~h, a restricted Ridge autoregressive benchmark attains the lowest pooled MAE (11.96~\textmu g/m\textsuperscript{3}; CI [10.96, 12.98]), marginally below jointly trained LSTMs (12.31--12.94~\textmu g/m\textsuperscript{3}).
...
% Research highlights
\begin{highlights}
\item Leakage-safe multi-horizon evaluation with exported test predictions, block-bootstrap 95\% CIs, and pre-specified paired $\Delta$MAE tests.
...
\item Paired tests: joint attention significantly outperforms joint LSTM at 7~d and 14~d (Table~\ref{tab:significance}); MAE$^{90}$ remains $\sim$3$\times$ pooled MAE (e.g., 14~d: 33.35 vs 9.78~\textmu g/m\textsuperscript{3}).
...
\begin{figure*}[t]
    ...
    \IfFileExists{Architecture.png}{%
        ...
        \fbox{\parbox[c][0.22\textheight][c]{0.95\linewidth}{\centering \textbf{Missing figure:} \texttt{figs/Architecture.png}}}%
    }%
...
24~h & Joint multi-horizon LSTM (mh\_lstm) & 12.31 & [9.99, 14.59] & 19.23 & 0.028 \\
24~h & Joint multi-horizon LSTM + attention (mh\_lstm\_mha) & 12.94 & [10.66, 14.87] & 18.30 & 0.120 \\
24~h & LSTM (per-horizon direct) & 23.19 & [19.92, 27.08] & 30.24 & $-1.344$ \\
24~h & Seasonal naïve (24h anchor) & 13.76 & [12.24, 15.10] & 21.94 & $-0.144$ \\
\midrule
7~d (168~h) & Joint multi-horizon LSTM + attention (mh\_lstm\_mha) & 11.32 & [9.68, 12.92] & 17.94 & 0.155 \\
7~d (168~h) & LSTM (per-horizon direct) & 13.04 & [11.92, 14.05] & 18.80 & 0.207 \\
7~d (168~h) & Joint multi-horizon LSTM (mh\_lstm) & 15.02 & [12.75, 16.59] & 21.55 & $-0.219$ \\
7~d (168~h) & Seasonal naïve (weekly anchor) & 15.39 & [13.40, 17.23] & 22.74 & $-0.226$ \\
\midrule
14~d (336~h) & Joint multi-horizon LSTM + attention (mh\_lstm\_mha) & 9.78 & [8.69, 10.80] & 16.58 & 0.108 \\
...
14~d (336~h) & Joint multi-horizon LSTM (mh\_lstm) & 16.58 & [14.72, 18.45] & 22.00 & $-0.571$ \\
\midrule
28~d (672~h) & Joint multi-horizon LSTM + attention (mh\_lstm\_mha) & 13.56 & [11.25, 16.35] & 19.98 & 0.041 \\
28~d (672~h) & Joint multi-horizon LSTM (mh\_lstm) & 14.92 & [13.41, 16.76] & 21.37 & $-0.097$ \\
...
\begin{figure}[H]
    \centering
    \IfFileExists{results_horizon_target_variability_box.png}{%
        \includegraphics[width=0.72\textwidth]{results_horizon_target_variability_box.png}%
    }{%
        \fbox{\parbox[c][0.12\textheight][c]{0.72\linewidth}{\centering \textbf{Missing:} \texttt{results\_horizon\_target\_variability\_box.png}}}%
    }%
...
\begin{itemize}
    ...
    \item We do not report systematic feature-importance analysis (e.g., SHAP or grouped ablations); interpretability of predictors remains for future work.
